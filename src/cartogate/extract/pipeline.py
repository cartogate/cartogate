...
def index_package(
    ...
    **Resilient to one bad file.** A file that can't be read is skipped; a structural-walk crash
    ...
    # --- Route pass: framework route declarations -> route nodes + links_to edges ---
    # EXTRACTED facts (file paths + string literals only), advisory by construction:
    # LINKS_TO is not in GATE_EDGE_TYPES. Feeds `cartogate navmap` + the drift advisory.
    from cartogate.extract.routes_js import extract_route_facts  # local: keep import lazy

    route_facts = extract_route_facts(
        root, repo_id=repo_id, base=base, allow=git_tracked_files(root)
    )
    if route_facts.nodes:
        # bulk_load is last-writer-wins PER UNIT: a route-declaring source file
        # already has a pending entry from the structural pass, so route facts
        # must MERGE into it, never append a second tuple under the same unit
        # (inspector Critical, 2026-07-20 — appended tuples silently clobbered
        # the file's symbol nodes in the store, blinding check_duplicate).
        route_by_unit: dict[str, tuple[list[Node], list[Edge]]] = {}
        for route_node in route_facts.nodes:
            unit_nodes, unit_route_edges = route_by_unit.setdefault(
                route_node.unit, ([], [])
            )
            unit_nodes.append(route_node)
            unit_route_edges.extend(
                e for e in route_facts.edges if e.src == route_node.id
            )
        pending_index = {unit: i for i, (unit, _n, _e) in enumerate(pending)}
        for unit, (r_nodes, r_edges) in route_by_unit.items():
            if unit in pending_index:
                i = pending_index[unit]
                existing_unit, existing_nodes, existing_edges = pending[i]
                pending[i] = (
                    existing_unit,
                    list(existing_nodes) + r_nodes,
                    list(existing_edges) + r_edges,
                )
            else:
                pending.append((unit, r_nodes, r_edges))
                units.append(unit)
        all_nodes.extend(route_facts.nodes)
        all_edges.extend(route_facts.edges)
